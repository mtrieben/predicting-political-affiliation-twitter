...
def prep_data(inputs_train, labels_train, inputs_test, labels_test, vocab):
    ...
    inputs_array_train = [x if len(x) >= max_len else x + ["<PAD>"]*(max_len - len(x)) for x in inputs_array_train]
    inputs_array_train = [[vocab[x] for x in y] for y in inputs_array_train]

    inputs_array_test = [x if len(x) >= max_len else x + ["<PAD>"]*(max_len - len(x)) for x in inputs_array_test]
...
if __name__ == "__main__":
    ...
    pkl_file.close()
    inputs_train, inputs_test, labels_train, labels_test, max_len = prep_data("data/train/text.npy", "data/train/labels.npy","data/test/text.npy", "data/test/labels.npy", vocab)
    ...
    testing = torch.utils.data.TensorDataset(inputs_test_tensor, labels_test_tensor)
